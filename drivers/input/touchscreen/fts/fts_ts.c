...
static struct fts_info *proc_ts;
...
#ifdef WISTRON_EXTENSION
	if(ts->client->addr == CTPM_OGS_I2C_ADDR) {
...
static void fts_early_suspend(struct early_suspend *h)
{
       ...
       printk("fts early suspend\n");
	int i;
	  struct fts_info *ts;
        ts = container_of(h, struct fts_info, early_suspend);
...
static int fts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts->early_suspend.suspend = fts_early_suspend;
	ts->early_suspend.resume = fts_late_resume;
