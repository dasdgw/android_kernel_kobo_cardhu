...
static int calc_h_ref_to_sync(const struct tegra_dc_mode *mode, int *href)
{
	...
	if (mode->h_ref_to_sync >= 0) //If panel driver already set value don't calculate it.
       *href = mode->h_ref_to_sync;
	else if (href) {
		if (b > a && a % 2)
		...
		const struct fb_videomode *fbmode, bool stereo_mode)
{
	...
	if (dc->out->type == TEGRA_DC_OUT_HDMI) {
		...
		if(dc->mode.h_ref_to_sync >= 0 && dc->mode.v_ref_to_sync >= 1){
			mode.h_ref_to_sync = dc->mode.h_ref_to_sync;
			mode.v_ref_to_sync = dc->mode.v_ref_to_sync;
		}
		else
