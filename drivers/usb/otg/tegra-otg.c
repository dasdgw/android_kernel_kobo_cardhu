...
extern struct completion phy_completion;
bool otg_irq = false;
				...
				enum usb_otg_state to)
{
	...
	struct tegra_udc *udc = container_of(otg->gadget, struct tegra_udc, gadget);
	...
	wait_for_completion(&phy_completion);
	phy_completion.done = 1;
...
static void irq_work(struct work_struct *work)
{
	...
	/* De-bouncing for VBUS status (USB_VBUS_STATUS)*/
	mdelay(25);
	...
	if (!(status & USB_ID_STATUS) && (status & USB_ID_INT_EN) && (status & USB_VBUS_STATUS) && from != OTG_STATE_A_HOST)
		to = OTG_STATE_B_PERIPHERAL;
	else if (!(status & USB_ID_STATUS) && (status & USB_ID_INT_EN) && from != OTG_STATE_B_PERIPHERAL)
		to = OTG_STATE_A_HOST;
