			      ...
			      unsigned long action, void *dev)
{
	...
	if (jack == &tegra_rt5640_hp_jack) {
		if (action) {
			...
			printk("[tegra_rt5640] machine->jack_status=%x, attach !!\n",machine->jack_status);
		} else {
			...
			machine->jack_status &= ~SND_JACK_HEADPHONE;
			machine->jack_status &= ~SND_JACK_MICROPHONE;
			printk("[tegra_rt5640] machine->jack_status=%x,   remove!! \n",machine->jack_status);
		}
	}
	...
	if (gpio_is_valid(pdata->gpio_ldo1_en)) {
		...
		if (ret){
			dev_err(&pdev->dev, "Fail gpio_request AUDIO_LDO1\n");
                        printk("Fail gpio_request AUDIO_LDO1\n");
                }
                ret = gpio_direction_output(pdata->gpio_ldo1_en, 1);
		if (ret){
			dev_err(&pdev->dev, "Fail gpio_direction AUDIO_LDO1\n");
                        printk("Fail gpio_direction AUDIO_LDO1\n");
                }
		msleep(200);
	}
	machine->pdata = pdata;
