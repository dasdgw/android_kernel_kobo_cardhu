...
#define GPIO_SW_KEY(_id, _gpio, _iswake, _can_disable)       \
	{					\
		.code = _id,			\
		.gpio = _gpio,			\
		.active_low = 1,		\
...
static struct gpio_keys_button cardhu_keys_e1291[] = {
	...
	[7] = GPIO_SW_KEY(SW_LID, TPS6591X_GPIO_5, 0, 1),
};
...
static struct gpio_keys_button cardhu_keys_e1291_a04[] = {
	...
	[3] = GPIO_SW_KEY(SW_LID, TEGRA_GPIO_PS5, 1, 1),
};

#define PMC_WAKE_STATUS 0x14

static int cardhu_wakeup_key(void)
{
	int wakeup_key;
	unsigned long status =
		readl(IO_ADDRESS(TEGRA_PMC_BASE) + PMC_WAKE_STATUS);

	if (status & ((unsigned long)1 << TEGRA_WAKE_GPIO_PV0))
		wakeup_key = KEY_POWER;
	else if (status & ((unsigned long)1 << TEGRA_WAKE_GPIO_PS6))
		wakeup_key = KEY_POWER;
	else if (status & ((unsigned long)1 << TEGRA_WAKE_GPIO_PS5))
		wakeup_key = SW_LID;
	else
