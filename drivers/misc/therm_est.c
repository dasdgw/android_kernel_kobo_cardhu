...
#ifdef CONFIG_PM
static bool tegra_skin_suspend;
static int tegra_skin_pm_notify(struct notifier_block *nb,
                                unsigned long event, void *data)
{
        switch (event) {
        case PM_SUSPEND_PREPARE:
                tegra_skin_suspend = true;//true;
                break;
        case PM_POST_SUSPEND:
                tegra_skin_suspend = false;
                break;
        }
...
#endif
static struct notifier_block tegra_skin_nb = {
        .notifier_call = tegra_skin_pm_notify,
};
			...
			long polling_period)
{
	...
	queue_delayed_work(est->workqueue,
				...
				msecs_to_jiffies(est->polling_period));
#ifdef CONFIG_PM
	register_pm_notifier(&tegra_skin_nb);
#endif
	return est;
}
