...
static int fts_enter_upgrade_mode(struct fts_info *ts)
{
	...
	for(k = 0; k < 3; k++){
		/******write 0xaa to register 0xfc******/
		fts_writeb(ts, 0xFC, 0xAA);

		mdelay(ts->fw_upgrade_info.delay_aa);
		
		/******write 0x55 to register 0xfc******/
		fts_writeb(ts, 0xFC, 0x55);
		...
		/********Step 3:check READ-ID********/
		fts_writesb(ts, check_id_cmd, sizeof(check_id_cmd));

		fts_readsb(ts, NULL, id, 2);
...
int fts_proc_create_touch(void)
{
	fts_create_proc(proc_ts);
}

EXPORT_SYMBOL(fts_proc_create_touch);
#endif
