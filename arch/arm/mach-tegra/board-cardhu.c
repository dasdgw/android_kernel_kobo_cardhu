...
#ifdef CONFIG_EEPROM_AT24
static struct i2c_board_info __initdata karen_i2c4_eeprom_board_info[] = {
       {
               I2C_BOARD_INFO("24c02", 0x54),
       },
};

static void karen_eeprom_init(void)
{
       i2c_register_board_info(4, karen_i2c4_eeprom_board_info,
               ARRAY_SIZE(karen_i2c4_eeprom_board_info));
}
#endif
...
static struct tegra_asoc_platform_data cardhu_audio_rt5640_pdata = {
...
#define TOUCH_GPIO_IRQ_FTS              TEGRA_GPIO_PH4
#define TOUCH_GPIO_RST_FTS              TEGRA_GPIO_PH6
unsigned TOUCH_GPIO_3V3_FTS=0;
static int fts_hw_power(int onoff)
{
	if(onoff) {
		gpio_direction_output(TOUCH_GPIO_3V3_FTS, 1);
		msleep(10);
	}
	else {
		gpio_direction_output(TOUCH_GPIO_3V3_FTS, 0);
	}
...
static void get_touch_pin(void){
  if(pcb_ver==PCB_SA||pcb_ver==PCB_SB)
  	TOUCH_GPIO_3V3_FTS=TEGRA_GPIO_PI3;
  else
  	TOUCH_GPIO_3V3_FTS=TEGRA_GPIO_PL5;
}
...
static int fts_touch_init(void)
{
	int ret;
	get_touch_pin();
	ret = gpio_request(TOUCH_GPIO_3V3_FTS, "ts-shdn");
	if (ret < 0) {
...
static struct i2c_board_info __initdata fts_i2c_info[] = {
	...
	{
		I2C_BOARD_INFO(FTS_NAME, CTPM_OGS_I2C_ADDR),
		.platform_data = &fts_pdata,
	},
	{
		I2C_BOARD_INFO(FTS_NAME, CTPM_GG_I2C_ADDR),
		.platform_data = &fts_pdata,
	}
#else
	{
...
static struct gpio_led karen_gpio_leds[] = {
	{
		.name = "white_led_en1",
		.gpio = TEGRA_GPIO_PL3,
		.active_low = 0,
...
static struct gpio_led_platform_data karen_gpio_led_info = {
	.leds = karen_gpio_leds,
	.num_leds = ARRAY_SIZE(karen_gpio_leds),
};

static struct platform_device karen_leds_gpio = {
	.name = "leds-gpio",
	.id = -1,
	.dev = {
		.platform_data = &karen_gpio_led_info,
	},
};

static void cardhu_leds_init(void)
{
	if(pcb_ver != PCB_SA) {
		if(pcb_ver == PCB_SB) {
			karen_gpio_leds[0].gpio = TEGRA_GPIO_PR3;
		}
		platform_device_register(&karen_leds_gpio);
	}
}
#endif
...
#endif
#ifdef CONFIG_TEGRA_BB_XMM_POWER2
...
{
#ifdef USE_CODEC_RT5640
	cardhu_audio_rt5640_pdata.codec_name = "rt5639.4-001c";
	cardhu_audio_rt5640_pdata.codec_dai_name = "rt5639-aif1";
#else	
	cardhu_audio_rt5640_pdata.codec_name = "rt5639.4-001c";
	cardhu_audio_rt5640_pdata.codec_dai_name = "rt5639-aif1";
#endif	
...
static void __init tegra_cardhu_init(void)
{
	...
	//There is no INA219 in karen board
