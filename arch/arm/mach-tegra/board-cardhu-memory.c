...
#define RAM_CODE_GMI_AD4 TEGRA_GPIO_PG4
#define RAM_CODE_GMI_AD5 TEGRA_GPIO_PG5
#define IS_APOLLOHD_BOARD 1

static const struct tegra_emc_table cardhu_emc_tables_karen_samsung[] = {
	{
		0x32,       /* Rev 3.2 */
...
static const struct tegra_emc_table cardhu_emc_tables_karen_hynix[] = {
	{
		0x32,       /* Rev 3.2 */
		25500,      /* SDRAM frequency */
		{
...
static const struct tegra_emc_table cardhu_emc_tables_karen_nanya[] = {
	{
		0x32,       /* Rev 3.2 */
		25500,      /* SDRAM frequency */
		{
...
int cardhu_emc_init(void)
{
	...
	ret = gpio_request(RAM_CODE_GMI_AD4, "RAM_CODE_GMI_AD4");

	if (ret < 0)
		return ret;

	ret = gpio_direction_input(RAM_CODE_GMI_AD4);
	if (ret < 0) {
		gpio_free(RAM_CODE_GMI_AD4);
		return ret;
	}

	ret = gpio_request(RAM_CODE_GMI_AD5, "RAM_CODE_GMI_AD5");
	if (ret < 0)
		return ret;

	ret = gpio_direction_input(RAM_CODE_GMI_AD5);
	if (ret < 0) {
		gpio_free(RAM_CODE_GMI_AD5);
		return ret;
	}

	ad4 = gpio_get_value(RAM_CODE_GMI_AD4);
	ad5 = gpio_get_value(RAM_CODE_GMI_AD5);	
	printk("[%s] AD4: %d , AD5: %d\n", __func__, ad4, ad5);

	if (ad5 == 0 && ad4 == 0) { // Samsung K4B4G1646B-HYK0
		printk("[%s] Apply ApolloHD_DVT_samsung_DVFS_table_0822\n", __func__);
		tegra_init_emc(cardhu_emc_tables_karen_samsung, ARRAY_SIZE(cardhu_emc_tables_karen_samsung));
	}
	else if (ad5 == 0 && ad4 == 1) { // Hynix H5TC4G63AFR-PBA
		printk("[%s] Apply AppoloHD_EVT_DVFS_Hynix_0603\n", __func__);
		tegra_init_emc(cardhu_emc_tables_karen_hynix, ARRAY_SIZE(cardhu_emc_tables_karen_hynix));
	}
	else if (ad5 == 1 && ad4 == 0) {
		printk("[%s] Apply AppoloHD_EVT_DVFS_Nanya_0611\n", __func__);
		tegra_init_emc(cardhu_emc_tables_karen_nanya, ARRAY_SIZE(cardhu_emc_tables_karen_nanya));
	}
	else if (ad5 == 1 && ad4 == 1) {
		// Reserved
	}
