...
static bool skip_pwr_event = false;
...
struct input_dev *ginput;
void report_power_event(void)
{	
	input_event(ginput, EV_KEY, KEY_POWER, 1);
	input_event(ginput, EV_KEY, KEY_POWER, 0);
	input_sync(ginput);
}
EXPORT_SYMBOL(report_power_event);
					   ...
					   const char *buf, unsigned int type)
{
    ...
    /* 
     * special check: if we're just setting to nothing, don't parse. this is
     * because bitmap_parselist will set_bit(0) when handed an empty list.
     * unfortunately - for us - SW_LID is bit(0), thus causing SW_LID
     * to be disabled, always.
     */
	...
	for (i = 0; i < ddata->n_buttons; i++) {
		...
		if (bdata->button->wakeup && device_may_wakeup(dev)) {
			...
			if (wakeup_key == bdata->button->code) {
				...
				if (bdata->button->code == SW_LID && state == 1) {
					continue;
				}
