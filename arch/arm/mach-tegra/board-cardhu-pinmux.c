...
static __initdata struct tegra_pingroup_config cardhu_pinmux_karen[] = {
	/* SDMMC1 pinmux */
	...
	DEFAULT_PINMUX(SDMMC4_RST_N,    RSVD1,           NORMAL,  NORMAL,     OUTPUT),
...
static __initdata struct tegra_pingroup_config cardhu_pinmux_touch_karen[] = {
	/* I2C2 pinmux */
	I2C_PINMUX(GEN2_I2C_SCL,	I2C2,		NORMAL,	NORMAL,	INPUT,	DISABLE,	ENABLE),
	I2C_PINMUX(GEN2_I2C_SDA,	I2C2,		NORMAL,	NORMAL,	INPUT,	DISABLE,	ENABLE),
};
...
static struct gpio_init_pin_info init_gpio_mode_karen[] = {
	...
	GPIO_INIT_PIN_MODE(TEGRA_GPIO_PB2, false, 0),
	GPIO_INIT_PIN_MODE(TEGRA_GPIO_PC1, false, 1),
	GPIO_INIT_PIN_MODE(TEGRA_GPIO_PL4, false, 1),
	/*Touch*/
	GPIO_INIT_PIN_MODE(TEGRA_GPIO_PI3, false, 0),
};
static struct gpio_init_pin_info init_gpio_mode_karen_sc[] = {
	GPIO_INIT_PIN_MODE(TEGRA_GPIO_PDD6, false, 0),
	GPIO_INIT_PIN_MODE(TEGRA_GPIO_PDD4, false, 0),
	GPIO_INIT_PIN_MODE(TEGRA_GPIO_PR2, false, 0),
	...
	GPIO_INIT_PIN_MODE(TEGRA_GPIO_PL4, false, 1),
	/*Touch*/
	GPIO_INIT_PIN_MODE(TEGRA_GPIO_PL5, false, 0),
};

void disable_touch_i2c_bus(void)
{
	/* Disable GEN2_I2C bus to save power */
	gpio_direction_output(TEGRA_GPIO_PT5, 0);
	tegra_pinmux_set_pullupdown(TEGRA_PINGROUP_GEN2_I2C_SCL, TEGRA_PUPD_NORMAL);
	tegra_pinmux_set_tristate(TEGRA_PINGROUP_GEN2_I2C_SCL, TEGRA_TRI_TRISTATE);
	gpio_direction_output(TEGRA_GPIO_PT6, 0);
	tegra_pinmux_set_pullupdown(TEGRA_PINGROUP_GEN2_I2C_SDA, TEGRA_PUPD_NORMAL);
	tegra_pinmux_set_tristate(TEGRA_PINGROUP_GEN2_I2C_SDA, TEGRA_TRI_TRISTATE);
}
EXPORT_SYMBOL(disable_touch_i2c_bus);

void enable_touch_i2c_bus(void)
{
	
	tegra_pinmux_config_table(cardhu_pinmux_touch_karen, ARRAY_SIZE(cardhu_pinmux_touch_karen));
			
}
EXPORT_SYMBOL(enable_touch_i2c_bus);
...
static void __init cardhu_gpio_init_configure(void)
{
	...
	struct gpio_init_pin_info *pins_info;
        ...
        if(pcb_ver==PCB_SA||pcb_ver==PCB_SB){
               len = ARRAY_SIZE(init_gpio_mode_karen);
	        pins_info = init_gpio_mode_karen;
        }else{
               len = ARRAY_SIZE(init_gpio_mode_karen_sc);
	        pins_info = init_gpio_mode_karen_sc;
...
int __init cardhu_pinmux_init(void)
{
	...
	tegra_pinmux_config_table(cardhu_pinmux_karen, ARRAY_SIZE(cardhu_pinmux_karen));
	tegra_drive_pinmux_config_table(cardhu_drive_pinmux, ARRAY_SIZE(cardhu_drive_pinmux));
...
struct gpio_init_pin_info pin_lpm_cardhu_karen[] = {
...
{
	set_unused_pin_gpio(&pin_lpm_cardhu_karen[0], ARRAY_SIZE(pin_lpm_cardhu_karen));
