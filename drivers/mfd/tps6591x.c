...
#define DEVCTRL_DEV_OFF_RST    (1 << 3)
...
#define TPS6591X_GPIO4_OFFSET  0x4	
#define GPIO_STS        0x2
...
#define PCB_SA 0
#define PCB_SB 1
#define WHITE_LED_GPIO 139
static struct i2c_client *tps6591x_i2c_client;

static void set_offcharging_mode_flag(int flag)
{
	...
	tps6591x_set_bits(dev, TPS6591X_BCK1_REG, flag);

//for debug
/*
   __tps6591x_read(tps6591x_i2c_client, TPS6591X_BCK1_REG,&val);	
   
	if (ret < 0)
		pr_info("%s:Validity reg write failed\n", __func__); 				
	else 
	{
		pr_info("%s:TPS6591X_BCK1_REG = 0x%x\n", __func__,flag); 	
	}
*/
}

static int sw_reset_notify(struct notifier_block *this, unsigned long code,
	void *unused)
{
	if (code == SYS_RESTART) {
		set_offcharging_mode_flag(1);
	}
...
static void tps6591x_power_off(void)
{
	...
	//Clear PWRON event interrupt status	
	__tps6591x_read(tps6591x_i2c_client, TPS6591X_INT_STS,&val);	

	if(val & PWRON_IT)	
		__tps6591x_write(tps6591x_i2c_client, TPS6591X_INT_STS,val|PWRON_IT);	

	set_offcharging_mode_flag(0);	
	...
	if(pcb_ver == PCB_SB) {
		...
		while(1)
		{
			if(gpio_get_value(WHITE_LED_GPIO))
				msleep(1);
			else
				break;
		}
	...
	__tps6591x_read(tps6591x_i2c_client, BBCH_REG,&val);
	val &=  ~BBCHEN;
	__tps6591x_write(tps6591x_i2c_client, BBCH_REG,val);	

	//Disable GPIO4 falling-edge detection interrupt
	__tps6591x_read(tps6591x_i2c_client, TPS6591X_INT_MSK3,&val);
	val |= GPIO4_F_IT_MSK;
	__tps6591x_write(tps6591x_i2c_client, TPS6591X_INT_MSK3,val);	
   
   __tps6591x_read(tps6591x_i2c_client, TPS6591X_GPIO_BASE_ADDR + TPS6591X_GPIO4_OFFSET,&val);	
	...
	{				
		if(!(val & GPIO_STS))
		{	
			dev_info(dev, "pcb ver = sa");
			dev_info(dev, "AC or USB is connected, Rebooting ... ");	
			tps6591x_set_bits(dev, TPS6591X_DEVCTRL, DEVCTRL_DEV_OFF_RST);	
		}
	}
	else
	{
		...
		otg_enable = gpio_get_value(OTG_ENABLE_PIN);
		
		if((val & GPIO_STS)&&(!otg_enable))
		{	
			dev_info(dev, "AC or USB is connected, Rebooting ... ");	
			tps6591x_set_bits(dev, TPS6591X_DEVCTRL, DEVCTRL_DEV_OFF_RST);	
		}
					...
					const struct i2c_device_id *id)
{
	...
	OTG_ENABLE_PIN = pdata->otg_enable_pin;

	ret = register_reboot_notifier(&notifier);
	if (ret) {
		dev_err(&client->dev, "cannot register reboot notifier\n");
	}
