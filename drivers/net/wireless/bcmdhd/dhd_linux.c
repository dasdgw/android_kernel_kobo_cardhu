...
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	char iovbufe[WL_EVENTING_MASK_LEN + 12];	/*  Room for "event_msgs" + '\0' + bitvec  */
	...
	if (dhd && dhd->up) {
		if (value && dhd->in_suspend) {
			...
			bcm_mkiovar("event_msgs", eventmask, WL_EVENTING_MASK_LEN, iovbufe, sizeof(iovbuf));
			if ((ret  = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, iovbufe, sizeof(iovbufe), FALSE, 0)) < 0) {
				DHD_ERROR(("%s read Event mask failed %d\n", __FUNCTION__, ret));
			}
			bcopy(iovbufe, eventmask, WL_EVENTING_MASK_LEN);
			...
			bcm_mkiovar("event_msgs", eventmask, WL_EVENTING_MASK_LEN, iovbufe, sizeof(iovbuf));
			if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbufe, sizeof(iovbufe), TRUE, 0)) < 0) {
